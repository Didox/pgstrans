class SequencialDstvsController < ApplicationController
  ...
  def index
    ...
    if params[:created_at].present?
      @sequencial_dstvs = @sequencial_dstvs.where("created_at >= ?", params[:created_at].to_datetime.beginning_of_day) 
      @sequencial_dstvs = @sequencial_dstvs.where("created_at <= ?", params[:created_at].to_datetime.end_of_day)
    end
